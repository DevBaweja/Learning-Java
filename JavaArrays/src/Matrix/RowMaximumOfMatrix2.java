...
// here in finding maximum we using element of array directly to store it into max 
// we can also use element of the matrix stored in them variable 

import java.util.Scanner;

public class RowMaximumOfMatrix2 {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<mat.length ;i++)
		{
			A[i] = mat[i][0] ; // assume 0 index is maximum
			for(int j=1; j<mat[i].length ;j++)
			{
				if( A[i] < mat[i][j] )
					A[i] = mat[i][j]; // finding maximum we are using element instead of index
			}
		}
