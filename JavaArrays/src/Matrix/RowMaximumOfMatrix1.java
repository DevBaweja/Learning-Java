...
// here in finding maximum we using element of matrix
...
public class RowMaximumOfMatrix1 {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<mat.length ;i++)
		{
			int max = mat[i][0] ; // assume 0 index is maximum
			for(int j=1; j<mat[i].length ;j++)
			{
				if( max < mat[i][j] )
					max = mat[i][j]; // finding maximum we are using element instead of index
			}
			A[i] = max;
		}
